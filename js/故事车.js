...
<!-- 第四个车和元 -->
// <script type="text/javascript">
$('.text-text-text').find('.img4').css('display', "none")
$('.text-text-text').find('.img4').eq(0).css('display', "block")

// $('.yuan').eq(0).css('bockground','#ffe400')
$('.yuan4').click(function() {
	$('.text-text-text').find('.img4').css('display', "none")

	$('.text-text-text').find('.img4').eq($(this).index()).css('display', "block")
	$('.yuan4').css('background', 'white')
	$(this).css("background", "#ffe400")
	$('.yuan4').css({
		transform: 'scale(1)'
	})
